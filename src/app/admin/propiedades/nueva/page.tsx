...
import { Save, ImageIcon, Loader2, Sparkles, Wand2, FolderSearch, MapPin, ExternalLink, FileText, FileSignature } from "lucide-react";
...
export default function NewPropertyPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        accessible: false,
        views: false,
        views_type: '', // mar, montaña, ciudad, despejadas, calle
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (result.success) {
                setSuccessMsg("¡Propiedad guardada correctamente! Redirigiendo al listado...");
                setTimeout(() => {
                    router.push("/admin/propiedades");
                    router.refresh();
                }, 1500);
            } else {
                let msg = result.error?.message || "Error al crear la propiedad";
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-6xl pb-32">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 leading-tight">Nueva Propiedad</h1>
                    <p className="text-sm text-slate-500">Completa la ficha técnica para publicar el inmueble.</p>
                </div>

                <div className="flex flex-wrap gap-2 w-full md:w-auto">
                    <Link href="/admin/propiedades" className="flex-1 md:flex-none justify-center px-4 py-2 border border-slate-300 text-slate-700 font-medium text-sm rounded-lg hover:bg-slate-50 transition-colors flex items-center">
                        Cancelar
                    </Link>
                    <button
                ...
                <div className="space-y-8">
                    <section id="section-1-datos" className={`bg-white p-6 rounded-xl shadow-sm border transition-all space-y-6 ${!formData.propietario_id ? 'border-orange-200' : 'border-slate-200'}`}>
                        <div className="flex justify-between items-center border-b pb-2">
                            <h2 className="font-bold text-lg text-slate-900">1. Operación y Datos Generales</h2>
                    ...
                    <section className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="mt-6 pt-6 border-t border-slate-100 md:col-span-2">
                    ...
                    <section className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-6">
                        <h2 className="font-bold text-lg text-slate-900 border-b pb-2">6. Multimedia Extras</h2>

                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">Galería de Imágenes</label>
                            <ImageGallery
                                images={formData.images}
                    ...
                    <div className="flex justify-center pt-8">
                        ...
                        >
                            ...
                            {loading ? 'Guardando...' : successMsg ? '¡Guardado con éxito! ✅' : 'Guardar Propiedad'}
                        </button>
                    </div>
                </div>
            </div>
